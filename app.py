...
if all([MYSQL_USER, MYSQL_PASS, MYSQL_HOST, MYSQL_PORT, MYSQL_DB]):
    ...
    # You could add a local SQLite URI here for development fallback, e.g.,
...
class Link(db.Model):
    ...
    # FIX 1 (CRITICAL): Renamed to 'name' to match usage in seeding and forms
    ...
    def __repr__(self):
        # This now correctly uses the 'name' attribute
...
def initialize_database():
    ...
    try:
        ...
        if db.session.execute(db.select(Link)).scalar() is None: 
            ...
            initial_links = [
                # FIX 1: 'name' is now correctly passed as a keyword argument matching the model
        ...
        # FIX 2: Define LOCAL_MYSQL_PASS to avoid NameError in logging if environment vars are empty
        LOCAL_MYSQL_PASS = "dummy_pass"
...
def admin_add_link():
    ...
    if request.method == 'POST':
        ...
        if link_name and link_category and link_url:
            new_link = Link(
                # FIX 1: The model property is now 'name'
                name=link_name,
...
def _create_swachatha_pdf_story(activity, region, warehouse, date_str_dd_mm_yyyy,description, image_paths):
    ...
    if image_paths:
        ...
        table_style = TableStyle([
            ...
            # Apply content_margin_left and right only to the outermost cells
...
def generate_pdf():
    ...
    try:
        ...
        for img in images:
            if img.filename != '':
                # Use a unique name to avoid conflicts if needed, but original code just uses filename
        ...
        return redirect(url_for('swachatha_form'))

    finally:
        # Clean up uploaded images regardless of success/failure
